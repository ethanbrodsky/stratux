...
// This is almost an exact copy of https://godoc.org/github.com/kidoman/embd/controller/pca9685
// but I had to modify it to add SetPrescaler(), as the setup() function was not doing it properly
// for PWM mode and thus the outputs weren't working unless I configured them manually.  It'd be
// better to have a function called SetFreqPWM() that calculates the prescaler byte.

// There are a number of other libraries for the PCA9685 that are worth considering
// https://godoc.org/periph.io/x/periph/experimental/devices/pca9685
// https://github.com/sergiorb/pca9685-golang
// https://github.com/goncalonsp/rpi-servo-pca9685
// The one by sergiorb looks most promising for being able to use as-is
...
func (d *PCA9685) SetPrescaler(prescalerValue byte) error {
	mode1Reg, err := d.mode1Reg()
	...
	if err := d.Bus.WriteByteToReg(d.Addr, mode1RegAddr, (mode1Reg & 0x7F) | 0x10); err != nil {
		return err
	}

	if err := d.Bus.WriteByteToReg(d.Addr, preScaleRegAddr, prescalerValue); err != nil {
		return err
	}

	if err := d.Bus.WriteByteToReg(d.Addr, mode1RegAddr, mode1Reg); err != nil {
		return err
	}
       
	time.Sleep(5 * time.Millisecond)

	if err := d.Bus.WriteByteToReg(d.Addr, mode1RegAddr, mode1Reg | 0x80); err != nil {
		return err
	}
